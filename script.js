...
//uso do day.js (formatador de datas - biblioteca)
...
const atividade = {
	nome: "Almoço",
	data: new Date("2021=07-08 10:00"),
	finalizada: false
...
//prever comportamento padrão do formulário
const salvarAtividade = (event) => {
    event.preventDefault()
    const dadosDoFormulario = new FormData(event.target)

    const nome = dadosDoFormulario.get('atividade') // é o name do input
    const dia = dadosDoFormulario.get('dia')
    const hora = dadosDoFormulario.get('hora')
    const data = `${dia} ${hora}`

    const novaAtividade = {
        nome: nome,  // poderia ser só "nome", quando é igual não precisa repetir
        data: data,
        finalizada: false    
    }
